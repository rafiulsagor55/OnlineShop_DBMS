...
const AdminTypeBasedItemUnisex = () => {
    ...
    const fetchFilters = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:8080/api/filters/get-filters?gender=${encodeURIComponent(
            myString
          )}`,
          {
            method: "GET",
        ...
        setDynamicFilterOptions({
          category: data.category?.length
            ? ["All", ...data.category, "Others"]
            : filterOptions.category,
          type: data.type?.length
            ? ["All", ...data.type, "Others"]
            : filterOptions.type,
          color: data.color?.length
            ? ["All", ...data.color, "Others"]
            : filterOptions.color,
          size: data.size?.length
            ? ["All", ...data.size, "Others"]
            : filterOptions.size,
          brand: data.brand?.length
            ? ["All", ...data.brand, "Others"]
            : filterOptions.brand,
          material: data.material?.length
            ? ["All", ...data.material, "Others"]
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/products/delete/${productToDelete.id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const sortCards = (cards, sortBy) => {
    switch (sortBy) {
      ...
      case "Newest First":
        return [...cards].sort((a, b) => parseInt(b.id) - parseInt(a.id));
      case "Discount: High to Low":
        return [...cards].sort((a, b) => b.discount - a.discount);
      case "Best Selling":
  ...
  const isAdminUnisexPage = location.pathname === "/admin/unisex";
  ...
  return (
    <div>
      ...
      {isAdminUnisexPage && (
        <div>
          <h2 className={styles.titleMain}>Admin Unisex Wear</h2>

          <div className={styles.searchContainer}>
            <input
              type="text"
              placeholder="Search by name, type, brand, or ID"
              value={search}
          ...
          <div className={styles.filterContainer}>
            ...
            <div className={styles.mobileFilterContainer}>
              <div
                className={`${styles.dropdownWrapper} ${styles.filterDropdownWrapper}`}
              >
                <input
                  type="checkbox"
                  id="filterToggle"
              ...
              <div
                className={`${styles.dropdownWrapper} ${styles.sortDropdownWrapper}`}
              >
                <input
                  type="checkbox"
                  id="sortToggle"
            ...
            {filtered.map((card) => (
              <div className={styles.customCard} key={card.id}>
                <div className={styles.cardContent}>
                  <h3 className={styles.cardTitle} title={card.name}>
                    {card.name}
                  </h3>
                ...
                <div className={styles.adminActions}>
                  <button
                    ...
                    onClick={() =>
                      navigate(`${card.id}`, { state: { id: card.id } })
                    }
                  >
                    View
                  </button>
...
export default AdminTypeBasedItemUnisex;